...
namespace Utils
{
    namespace FileSystem
    {
        ...
        // A directory listing entry that also carries the entry type, which can be obtained
        // from the directory iterator without issuing an additional stat() syscall for the
        // common case. This lets callers that scan large collections avoid repeated stat()
        // calls on every file.
        struct DirEntry {
            std::string path;
            bool directory;
