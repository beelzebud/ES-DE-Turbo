...
namespace Utils
{
    namespace FileSystem
    {
        ...
        std::vector<DirEntry> getDirContentEntries(const std::string& path)
        {
            const std::string& genericPath {getGenericPath(path)};
            std::vector<DirEntry> contentList;
            ...
            try {
                ...
                for (auto& entry : std::filesystem::directory_iterator(
                    ...
                    dirEntry.path =
                        Utils::String::wideStringToString(entry.path().generic_wstring());
                    dirEntry.directory = entry.is_directory();
                    dirEntry.symlink = entry.is_symlink();
                    contentList.emplace_back(std::move(dirEntry));
                }
#else
                for (auto& entry : std::filesystem::directory_iterator(genericPath)) {
                    DirEntry dirEntry;
                    dirEntry.path = entry.path().generic_string();
                    dirEntry.directory = entry.is_directory();
                    dirEntry.symlink = entry.is_symlink();
                    contentList.emplace_back(std::move(dirEntry));
                }
...
#if defined(_WIN64)
                LOG(LogError) << "FileSystemUtil::getDirContentEntries(): Couldn't read directory "
                              << "\"" << Utils::String::replace(path, "/", "\\")
                              << "\", permission problems?";
#else
                LOG(LogError)
                    << "FileSystemUtil::getDirContentEntries(): Couldn't read directory " << "\""
                    << path << "\", permission problems?";
#endif
                return contentList;
            }
            ...
            std::sort(contentList.begin(), contentList.end(),
                      [](const DirEntry& a, const DirEntry& b) { return a.path < b.path; });
            return contentList;
        }
