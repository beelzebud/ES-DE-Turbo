...
bool SystemData::populateFolder(FileData* folder)
{
    ...
    // Fetch the directory listing together with the entry types. This lets us avoid
    // issuing isDirectory()/isSymlink() stat() calls for every file, which is a major
    // cost when scanning large collections.
    const std::vector<Utils::FileSystem::DirEntry> dirContent {
        Utils::FileSystem::getDirContentEntries(folderPath)};
    bool isGame {false};
    ...
    const std::string noLoadPath {mEnvData->mStartPath + "/noload.txt"};
    const std::string flattenPath {mEnvData->mStartPath + "/flatten.txt"};
    bool noLoadFound {false};
    bool flattenFound {false};
    for (const auto& entry : dirContent) {
        if (entry.path == noLoadPath)
            noLoadFound = true;
        else if (entry.path == flattenPath)
            flattenFound = true;
    }
    if (noLoadFound) {
    ...
    for (const auto& entry : dirContent) {
        filePath = entry.path;
        // Symlinks still need a status() call that follows the link, but for regular
        // entries the type is already known from the directory iterator (no syscall).
        const bool isSymlink {entry.symlink};
        const bool isDirectory {isSymlink ? Utils::FileSystem::isDirectory(filePath) :
                                            entry.directory};
        ...
        if (mEnvData->mSearchExtensionsSet.count(extension) != 0 &&
...
bool SystemData::loadConfig()
{
    ...
    for (auto& configPath : configPaths) {
        ...
        for (pugi::xml_node system {systemList.child("system")}; system;
            ...
            envData->mSearchExtensionsSet.clear();
            envData->mSearchExtensionsSet.insert(extensions.cbegin(), extensions.cend());
