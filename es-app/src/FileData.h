...
public:
    ...
    // Cached uppercase key used for name-based sorting. It is recomputed only when the
    // underlying metadata changes, which avoids repeatedly uppercasing the same strings
    // on every comparator invocation when sorting large collections.
    const std::string& getSortNameKey() const;
...
private:
    ...
    bool mDeletionFlag;
    bool mNoLoad;
    // Cache for getSortNameKey(); sources are stored so the key can be invalidated
    // when the metadata used to compute it changes.
    mutable std::string mSortNameKey;
    mutable std::string mSortNameKeySrc1;
    mutable std::string mSortNameKeySrc2;
    mutable std::string mSortNameKeySrc3;
};
