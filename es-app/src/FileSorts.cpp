...
namespace FileSorts
{
    ...
    {
        // The uppercase sort key is cached per file, which avoids re-uppercasing the
        // metadata strings on every comparison when sorting large collections.
        return file1->getSortNameKey() < file2->getSortNameKey();
    }
