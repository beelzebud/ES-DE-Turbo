...
const std::string& FileData::getSortNameKey() const
{
    std::string src1;
    std::string src2;
    std::string src3;

    if (mSystem->isCustomCollection()) {
        src1 = metadata.get("collectionsortname");
        src2 = metadata.get("sortname");
        src3 = metadata.get("name");
    }
    else {
        src1 = metadata.get("sortname");
        src2 = metadata.get("name");
    }

    if (src1 != mSortNameKeySrc1 || src2 != mSortNameKeySrc2 || src3 != mSortNameKeySrc3) {
        mSortNameKeySrc1 = src1;
        mSortNameKeySrc2 = src2;
        mSortNameKeySrc3 = src3;

        std::string key {Utils::String::toUpper(src1)};
        if (key.empty())
            key = Utils::String::toUpper(src2);
        if (key.empty() && mSystem->isCustomCollection())
            key = Utils::String::toUpper(src3);
